...
if ($ID) {
    ...
    if ($result->num_rows > 0) {
        ...
        $mobile = htmlspecialchars($row['Tel']);
        $email = htmlspecialchars($row['Email']);
        $pword = htmlspecialchars($row['Password']);
    } else {
...
<form id="recipe" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
    <div id="formdiv">
        ...
        <input type="text" class="formele" id="mobile" name="mobile" required value="<?php echo htmlspecialchars($mobile); ?>"><br><br>
        <label class="formele">Email:</label><br><br>
        <input type="email" class="formele" id="email" name="email" required value="<?php echo htmlspecialchars($email); ?>"><br><br>
...
if (isset($_POST["submit"])) {
    ...
    $stmt = $con->prepare("UPDATE `user` SET `Fname` = ?, `Tel` = ?, `Email` = ?, `Password` = ? WHERE `ID` = ?");
